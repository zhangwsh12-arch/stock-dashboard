#!/usr/bin/env node
/**
 * 更新 content.json：补充 6/4 和 6/5 的 events 和 industryNews
 */
...
const contentFile = join(__dirname, '..', 'data', 'content.json');
...
// 插入到 events 数组（NC 6/2 条目之后）
const events = content.events;
let insertIdx = -1;
for (let i = 0; i < events.length; i++) {
  if (events[i].date === '06.02' && events[i].company.includes('NC')) {
    insertIdx = i;
    break;
  }
}
if (insertIdx >= 0) {
  ...
  console.log(`⚠️ 未找到 NC 6/2 插入位置，已 push 到末尾，共 ${events.length} 条`);
}
...
console.log(`✅ content.json 已保存`);
console.log(`   updatedAt: ${content.meta.updatedAt}`);
